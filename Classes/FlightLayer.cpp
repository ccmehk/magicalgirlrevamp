...
void FlightLayer::initTeam(const HeroMessage& h1){

	Hero* hero1 = Hero::create(h1.r_name, this);
	hero1->setPosition(-100, 260);
	hero1->setDesPoint(Point(400, 260));
	hero1->initWithMessage(h1);
	this->addRole(hero1);

}

void FlightLayer::initTeam(const HeroMessage& h1, const HeroMessage& h2){
	initTeam(h1);
